...
(defun a*-search-time (open-list goal-p successors cost-fn remaining-cost-fn closed-list)
	(loop
	 (setq time1a (get-internal-run-time))
	 (setq time2a (get-internal-run-time))
	 (format t "Calibration takes ~f msec of run time~%"
		(- time2a time1a))
	 (setq time2a (get-internal-run-time))
	 (format t "Calibration takes ~f msec of run time~%"
		(- time2a time1a))
	 (setq time1b (get-internal-run-time))
     (when (pq-empty open-list) (return nil))
     (when (funcall goal-p (my_path-state (pq-peek open-list)))
       (return (pq-peek open-list)))
     (when (pq-empty open-list) (return nil))
	   ...
	   (let* ((cost (+ (my_path-cost-so-far my_path)
	     ...
	     (pq-add open-list
		     (make-my_path
		      ...
		      :total-cost total-cost) 
		     total-cost)))))
	(setq time2b (get-internal-run-time))
	(format t "Inner loop takes ~f msec of run time~%"
		(- time2b time1b))	
	)
	
)

(defun a*-time (start-state goal-p successor-fn heuristic-fn)
  (setf expanded 0)
  (setf generated 0)
  (setf rcalls 0)
  (setf getcalls 0)
